...
> **头号勘误**：任务书写的分支头是 `94c130d6`，而写报告时 `git rev-parse HEAD` 读到的是
> `4e7bf0fe`（`94c130d6` 是它的祖先，另有 `872370ba` 一个提交夹在中间，两者都是 P6.5 的文档/传输
> 提交，不涉及本次改动）。任务书同时把 `94c130d6` 标为门 8 的源码头——本文件里的源码头就是这样记的，
> 与分支头无关。

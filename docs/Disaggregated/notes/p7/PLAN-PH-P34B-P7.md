...
| PH-2 slot 预算（`SlotTables.h:347/:371` 两条 `MOBILEGL_ASSERT`） | partial | 必须 | 提升为 release 下具名拒绝（先经 `SessionFail`，闩落地后改闩） | 0.5 天 | 与 D11 五处一起 |
...
| 门：非 loopback 无令牌具名拒绝 | done（机制）⚠ | 必须 | 见 PH-7 (2)：名字不在词表；`tcp_supervisor_smoke.py` **未接 CI**（全仓无引用） | 含在 PH-7 | 同期 |
| OQ-21 令牌够不够 / TLS | 未答 | 必须裁定 | 见 ID-P7-3 | 0 | 本文 |

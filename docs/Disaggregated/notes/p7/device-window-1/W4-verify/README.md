...
4. **两进程臂同样掷硬币**（spawn 3 遍 红/绿/红），且第三张图 `4e5ba514`（9836 像素）在 run-ahead 下也出现。**裁判裁定（四镜头 + 对抗裁判，`~/w7/notes/evidence/w4/judge.json`，置信 0.9）**：两张错图都是 call 30376 顶点范围的**前缀撕裂**——`fb75d412` 整段 11448 B 丢、`4e5ba514` 前 6696 B（q00..q30）丢，正好是上一帧对 buffer 1 写入序列的后缀极大值。机制 H1：帧 N-1 的第一次 pooled-fence 中帧提交 S1（调色板 `glTexImage2D` → `FlushWirePendingCommandsForTextureUpdate` → `FlushPendingCommands`）退休时 `OnSubmitsCompletedUpTo:13483` 对该记录调 `NotifyFrameSerialComplete(N-1)`，而携带 staged `vkCmdCopyBuffer` 的 Present 提交 S2(N-1) 还在执行 → floor 无证据地升到 N-1 → 帧 N 对 buffer 1 的第一次写（call 30366）在 `WriteWireBuffer:254` 判「上次使用已完成」、走 `:268` 的无序 host memcpy → 被 S2(N-1) 排队的拷贝盖掉。主机 lavapipe 实测 split 每跑 26 次不实的 serial-complete、154 次 host 写，monolith 0。E5（`FRAMESINFLIGHT=4/8` 各 3 遍全红）与之一致：GPU 落后越多，memcpy 被盖得越彻底。修复交 **B3**（floor 只升到可证值 + 三处单 fence 退休站点共用一个 gather-wait + `:254` 兼看 `gpuWritesPending`；red-once = 计数探针 + `MOBILEGL_TEST_WIRE_GPU_DRAG_FILLS` 拖慢 Present 提交的 spawn 车道用例）。镜头 A 的「静默丢 draw」候选被裁判**否决**（每个 `SetupDraw` 出口都有 `MGLOG_E_ONCE`，且 `4e5ba514` 只丢一次 draw 的前 31 个 quad）。

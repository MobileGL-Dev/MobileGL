...
### 2.13 R3 落地:texture respecify 也只在首次使用时付一次——那一帧 130.9 s → 31.3 s

实现:把"这一行要不要等"做成**唯一一处**判据 `MGPipeResourceRespecifyWantsItsReply`（`MG_Pipe/PipeRoute.h`），wire 端读它决定要不要开回复槽：
buffer 半永远不等（两处返回值都被丢弃）；texture 半只在**对象未确认**时等（`MGPipeHandleIsPublished` 为假），确认本身由 `EmitResourceRespecify` 在首次使用时补的那次**阻塞 create** 付掉。

同一台设备、同一份工作量（staged **101,421,396 B**，与改动前逐字节相同）、出厂省电、服务端 spin 默认：

| | 那一帧 waits | `by_op`（等待） | `by_op`（发出的记录） | 墙钟 |
|---|---|---|---|---|
| R1+R2 之后 | 25,154 | `2=4536, 3=20611, 5=6, 9=1` | `2=4536, 3=20633, …` 共 103,088 | 130.9 s |
| **R3 之后** | **4,543** | `2=4536, 5=6, 9=1` | `2=4536, `**`3=20633`**`, …` 共 **103,094** | **31.3 s** |

**respecify 那 20,633 条记录一条没少（照发），少的是它们各自的那次往返**——这正是这条路线与"少干活"的区别。
独立审查给的预测是"4,543 waits、24–37 s（中点 ~30 s）"，实测 **31.3 s**，落在中点。

**累计（同一帧）**：43,232 waits / 221.5 s → **4,543 waits / 31.3 s**（7.1×）。剩下的是 create 那 4,536 次等待 ≈ 23 s，
那是确认本身，不是浪费；硬地板还有客户端 CPU ~13.4 s 与传输 ~2.2 s（审查实测 43.1 MiB/s）。**想再往下只剩"批处理 create 行"**（4,536 → ~1.1k，且仍拿真答案）。

**两笔明账**：
1. **① 的守卫尚未接**：审查独立复核"今天不可达"（能清纹理记录的只有 `MGPipeApplierReleaseObjectRecords`，调用者只有测试；`MGPipeApplierReset` 不清它）。
   接管时**优先查能否挂在 caps 代次/会话边界这类已有信号上**（一行）；否则要新增反向通道第 10 个回调 = 协议面 = 两端重出 APK。
2. **画面未验**：这条路线改的是"客户端还问不问"，`frame-sent` 一致是强证据但不是画面证据；R3 之后应补一次 SSIM/目视。

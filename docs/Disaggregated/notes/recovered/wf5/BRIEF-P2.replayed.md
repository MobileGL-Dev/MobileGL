...
**Tree creation.** `bash scratchpad/wf4/wsl_p1_tree.sh` copied to `wsl_p2_tree.sh` with `p1`→`p2` throughout, then per slug:
...
# benchmark (package E and the integrator only; DriverBench needs it and it is Linux-desktop only)
cmake -S . -B build-bench  ... same ... -DMOBILEGL_BUILD_BENCHMARK=ON
```

### C.0 Package A — `p2/contract` then `p2/spans` (tree `~/w7/p2-contract`, `~/w7/p2-spans`)
...
| MODIFY | `MobileGL/MG_Pipe/PipeFields.def` | D3.2 — three rows in `MGP_FIELDS_RenderStateParameters` (`:230-247`); `MGP_FIELDS_ResidualValueBlock` (`:147-148`) drops to `F(CapabilityBits)`; `MGP_COVERAGE_EMITTED_LIST` consumers (D5) |
...
| MODIFY | `scripts/gen_pipe.py` | extend `PIPELINE_STATE_MEMBERS` (`:75-99`) to the D6 set and rewrite `PipeSpanTable.inc`'s "deliberately absent" block (`:1045-1058`) to record what P2 decided; emit the `ResidualValueBlock` `offsetof` asserts; emit `kMGPipeFieldEmittedBy[]` from `MGP_COVERAGE_EMITTED_LIST` |
...
| MODIFY | `CMakeLists.txt` | the three new sources inside `if (MOBILEGL_PIPE_PUSH)` (`:462-468`); `option(MOBILEGL_PIPE_LEGACY_MEMOS … ON)` + `-DMOBILEGL_PIPE_LEGACY_MEMOS=1` (D14), forced ON with a `message(STATUS)` when `MOBILEGL_PIPE_PUSH=OFF` |
...
| MODIFY | `MobileGL/MG_Util/Metrics/PipeStats.{h,cpp}` | D17 — `CallClass::{RenderStateCsoMints, RenderStateCsoBinds}` + short names `csom`/`csob` |
| MODIFY | `MobileGL/MG_Test/Util/PipeStatsTest.cpp` | pin the two new names in `CounterNamesAreStable` (`:260`) and the summary-line test (`:137`) |
...
Ordered steps for `c0`: value types and `RenderState` first (build pull, confirm `sizeof` still 1168 and `symbol_report --threshold 0` shows only the three named resizes) → `MGPipeTypes.h`/`PipeFields.def`/generator/regenerate → the three new source files with complete signatures and *stub bodies for `PipeApply`'s derivation* (so B/C/D link) → CMake/Config → tests. Then **tag `p2/contract`** and tell the other packages.
...
**Then `p2/spans` continues** with the work that only A can do:
...
- `c3`: `SlotAllocatorTest.cpp` — slot reuse bumps `Gen` and only slot reuse does; a freed slot is handed back before the high-water grows; `lifetimeId → slot` survives a recycled heap address (the `void* volatile` sink of `ObjectLifetimeIdTest.cpp:44`); `kMGPipeFirstAllocatableSlot` is never handed out as 0; the composite `ShaderCso` band (`MGPipeHandles.h:83-97`) is not handed out by the ordinary allocator. Message: `[Test] (Pipe): pin the slot allocator's identity contract - gen moves only on reuse and a recycled address never reproduces a handle`.
...
bash scripts/g7_negative_control.sh build-push     # must report "negative control tripped"
git show $BASE:MobileGL/MG_Backend/DirectGLES/DirectGLES.cpp | awk '/^    namespace RenderStateImpl \{/,/^    \} \/\/ namespace RenderStateImpl/' | sha256sum
awk '/^    namespace RenderStateImpl \{/,/^    \} \/\/ namespace RenderStateImpl/' MobileGL/MG_Backend/DirectGLES/DirectGLES.cpp | sha256sum   # equal: G5
```

### C.1 Package B — `p2/tracker` (tree `~/w7/p2-tracker`, branch `p2/tracker`, from `p2/contract`)
...
| MODIFY | `MobileGL/MG_Impl/Pipe/PipeFill.{h,cpp}` | D1 — `MGPipeFillForVerb` → `MGPipeValidateForVerb`, the five-step body, the residual fill restricted by `kMGPipeFieldEmittedBy[]`; the `MGP_FILL` macro's expansion |
| MODIFY | `MobileGL/MG_Pipe/PipeMutation.h` | D4 — `MGP_NOTE_AGGREGATE(Aggregate)`, `((void)0)` in the pull build |
| MODIFY | `MobileGL/MG_State/GLState/{VertexArrayState/VertexArrayState.h, FramebufferState/FramebufferState.h, TextureState/TextureState.h, BufferState/BufferState.h}` and their `.cpp`s | D4 — the five aggregate generations and their ~20 bump lines, all `#if MOBILEGL_PIPE_PUSH` |
| MODIFY | `MobileGL/MG_State/GLState/{VertexArrayState/VertexArrayObject.cpp, FramebufferState/FramebufferObject.cpp, TextureState/{TextureObject,TextureObject2DCube}.cpp, SamplerState/SamplerObject.cpp, BufferState/BufferObject.cpp}` | the `MGP_NOTE_AGGREGATE` statements at the bump points listed in D4 |
...
| MODIFY | `MobileGL/MG_Pipe/FillPoints.def` | tighten the fill table: re-check the **8 statically over-approximated rows** (`MEASUREMENTS.md:115`) named in the three groups at `MEASUREMENTS.md:114` before narrowing anything, and record the verdict per row in the def's comment |
| MODIFY | `MobileGL/MG_Test/Pipe/{TrackerTest,CsoCacheTest}.cpp` | contents (the stubs are A's) |

**Ordered steps**

1. `b1`: the five aggregate generations + `MGP_NOTE_AGGREGATE`, with a unit test that each bump point moves its aggregate and nothing else. Build pull, `symbol_report --threshold 0` unchanged. Message: `[Feat] (State): give the frontend five aggregate generations so a tracker can answer "did any bound texture/buffer/attachment/attribute move" with one Uint64 compare`.
2. `b2`: `Tracker.{h,cpp}` + the `MGPipeValidateForVerb` rewrite, emitting **nothing** yet (dirty computed, counted, then the full P1 residual fill runs). Push build green on `unit` and `integration-gpu`; verify build green. This step is the safety net: it proves the walk costs nothing semantically before anything is removed. Message: `[Feat] (Pipe): compute the per-verb dirty mask at the validate point and count how often each bit fires`.
3. `b3`: `CsoCache` + `create/bind_render_state` + `set_dynamic_state` emission, and the residual fill drops the 29 derived fields. **This is where the render-state family stops being pulled.** Message: `[Feat] (Pipe): mint render-state CSOs on the pipeline subset and send only the changed dynamic chunks - the steady-state cost of the whole render-state family is now two Uint16 compares`.
...
5. `b5`: `set_residual_value_state` + the tripwire (D9). Message: `[Feat] (Pipe): carry what has no call yet in the residual value block and abort when it disagrees with the assembled state`.
...
**Tests to add**
- `TrackerTest.SteadyStateEmitsNothing`: two identical draws in a row emit zero calls (assert the `PipeStats` call counters do not move).
- `TrackerTest.BlendToggleReusesTwoCsos`: `enable/draw/disable/draw` × N mints exactly **2** CSOs and issues 2N binds — the Blaze3D shape (`ARCHITECTURE.md:151`).
- `TrackerTest.ViewportDoesNotMintACso`: `glViewport` in a loop mints 0 CSOs and emits N `set_dynamic_state`s carrying only chunk D0 — the regression `RenderState.h:165-170` records.
- `TrackerTest.WrapAroundRePushesButNeverMisses`: drive `m_version` past 65535 and assert no missed emission.
- `TrackerTest.AggregateGenerationCatchesABoundTextureMoving`: bind a texture, draw, `glTexSubImage` it, draw again — `NEW_SAMPLER_VIEWS` fires on the second draw. (This is the "dirty bit that fires too rarely" direction, which the P1 verify comparator **cannot** see because it compares O-class fields by identity only, `PipeInputs.h:248-249`.)
- `CsoCacheTest.LruEvictsTheOldestAndEmitsDelete`; `CsoCacheTest.HashCollisionDoesNotAliasTwoStates` (inject a forced collision through a test seam and assert the `memcmp` rejects it); `CsoCacheTest.ContentAddressingOffMintsEveryTime`.

**Constraints**: B touches **no** file under `MG_Backend/`. B does not edit `MG_Pipe/{MGPipeValueTypes.h, MGPipeTypes.h, MGPipeRenderStateSpans.*, PipeApply.*}` or `scripts/gen_pipe.py` (A owns them). B does not edit `.github/workflows/test.yml` (E owns it) — the CI step for the dirty-surface gate is E's, and B's scanner change must therefore be backward compatible with `--summary` until E lands.
...
# the per-subsystem A/B, which must be pixel-identical to the all-pull arm:
MOBILEGL_PIPE_PUSH=0 ctest --test-dir build-push -L integration-gpu --no-tests=error -j 4
```

### C.2 Package C — `p2/espryt` (tree `~/w7/p2-espryt`, branch `p2/espryt`, from `p2/contract`)

**Files**: `MobileGL/MG_Backend/DirectGLES/{DirectGLES.cpp, Managers.h, Managers.cpp}`, new `MobileGL/MG_Backend/DirectGLES/SlotTables.h`, and `MobileGL/MG_Test/SanityTest.cpp` (the `ScopedDirectGLESTextureBindings` fixture only).

**Ordered steps**

1. `e1`: `SlotTables.h` — per-kind dense arrays with a live bitset, save/reset/restore, and the `EnsureProcessTeardownSentinel()` arming moved onto first insertion. Both arms compile; nothing uses the new one yet.
2. `e2`: the explicit-destroy hook — `delete_*` emitted when a frontend object's last `SharedPtr` drops, following `BufferBackendOps`' shape (`BufferObject.h:76-103`, `BufferObject.cpp:26-41`). This must land **before** the tables are switched over, because the tables have no GC.
3. `e3`: switch the six kinds over behind `kMGPipeSubsystemEsprytSlots`, deleting `OwnerEquals`, the three `TwinLookupMemo`s, `UnitSamplerLookupMemo`'s `WeakPtr` test, the seven `CollectGarbageIfNeeded` calls, `SyncTextureObjectToBackend`'s by-value copy and second `Find`, and the four `defaultFBO` identity compares (D13). Keep the legacy arm under `MOBILEGL_PIPE_LEGACY_MEMOS`.
4. `e4`: `g_fbSlotCache` / `GetFramebufferBindingSlotFast` → a plain `MGB_CTX->GetFramebufferBindingSlot(target)` read at all five call sites (`DirectGLES.cpp:1613, 1907, 2745, 2860, 2935`), closing the P1 poison bypass.
...
**Must not break** — the D13 list; each item has a named existing test (`SanityTest.cpp:2575-2596, 2650-2665, 2757-2790, 3020-3062`; `Scenarios/{SampledSetStalenessScenario, CrossFrameBufferScenario, VertexArrayEnableDisableScenario, VertexAttribBindingScenario, MultiDrawScenario, ResidentIndexScenario}.cpp`).

**Verification (`~/w7/p2-espryt`)**
```sh
cmake --build build-linux --parallel 28 && python3 scripts/symbol_report.py --before ~/w7/p2-before-libMobileGL.so --after build-linux/libMobileGL.so --threshold 0
grep -rc 'pGLContext' MobileGL/MG_Backend | grep -v ':0$'        # empty (G13)
grep -c 'OwnerEquals\|TwinLookupMemo\|g_fbSlotCache\|CollectGarbageIfNeeded' MobileGL/MG_Backend/DirectGLES/DirectGLES.cpp
#   -> only the occurrences inside the MOBILEGL_PIPE_LEGACY_MEMOS arm
...
MOBILEGL_PIPE_PUSH=0 ctest --test-dir build-push -L integration-gpu --no-tests=error -j 4 -R 'DirectGLES'   # legacy arm, same names, same result
...
### C.3 Package D — `p2/magma` (tree `~/w7/p2-magma`, branch `p2/magma`, from `p2/contract`)
...
1. `m1`: re-key `GetOrCreatePipeline`'s memo on the bound CSO handle and delete the cached-hash fields (D12.1–12.2), behind `kMGPipeSubsystemRenderState`. Message: `[Refactor] (Magma): key the pipeline memo on the render-state CSO handle and stop recomputing a hash the client already computed`.
2. `m2`: `ApplyDynamicDrawStateTail` re-sourced (D12.3) plus the `DynamicTailKey`-coverage unit test. Message: `[Refactor] (Magma): drive the dynamic tail from the pushed dynamic version so a pipeline-only change stops invalidating it`.
3. `m3`: subsystem 4 — `ComputeHash`'s `bufferKey`, `VaoDrawMemo` slot-indexed, behind `kMGPipeSubsystemMagmaVertexInput` (D12.4). Message: `[Refactor] (Magma): key the vertex-input cache and the VAO draw memo on {slot, gen} instead of a lifetime id and a heap address`.
4. `m4`: delete `SetBackendStateMemo`/`SetBackendAuxMemo`/`SetBackendHashMemo` and their storage from the frontend VAO, and the eviction-epoch dance with them (D12.5). Message: `[Refactor] (State, Magma): take the backend's raw pointers out of the frontend VAO - the state memo is deleted, the hash and aux memos become server-side per-slot fields`.
...
### C.4 Package E — `p2/gates` (tree `~/w7/p2-gates`, branch `p2/gates`, from `p2/contract`)

Cross-cutting tooling. E branches from the contract and lands **last**, but writes `HandleRecycleScenario` **first** so that its `AbaControl` arm is proven red before C and D exist — that is the "重键前红" evidence.

| action | file | what |
|---|---|---|
| CREATE | `MobileGL/MG_IntegrationTest/Scenarios/HandleRecycleScenario.cpp` | D18 |
| CREATE | `MobileGL/MG_IntegrationTest/Scenarios/CsoContentAddressingScenario.cpp` | G12 — two entries, bit set vs clear, asserting `RenderStateCsoMints`/`Binds` move in the expected direction |
| MODIFY | `MobileGL/MG_IntegrationTest/CMakeLists.txt` | register both scenarios (source list from `:55`) and the five environments, following `:695-725` |
| MODIFY | `android-plugin/app/src/trace/cpp/trace_benchmark.{hpp,cpp}` | D.4.1 — a second per-frame series, `CLOCK_THREAD_CPUTIME_ID`, alongside the wall clock at `trace_benchmark.cpp:74-77`; `Report` gains `std::vector<double> frameCpuMs` next to `frameMs` (`trace_benchmark.hpp:32-39`) |
...
| MODIFY | `MobileGL/MG_Benchmark/Driver/CMakeLists.txt` | a second ctest entry `DriverBenchStateToggle` running `mc_state_toggle` (`:14` currently runs only `draw_tiny`), `LABELS benchmark` |
| CREATE | `scripts/g7_negative_control.sh` | D19 |
| MODIFY | `.github/workflows/test.yml` | `pipe-gates`: dirty-surface `--check` + `--self-test` in place of `--summary` (`:1524-1528`); a `ctest -L unit` step on the verify runtime in `integration-verify` (so G6/G10's push-only unit tests actually run in CI); the two new scenario entries |
...
#   on the contract tree, before C/D: .Legacy and .AbaControl must pass, .Handles must SKIP (subsystem not implemented)
cmake --build build-bench --parallel 28 && ctest --test-dir build-bench -L benchmark --no-tests=error
cd tools/trace_replay && cmake --build ../../build-retrace && ./build-retrace/trace_replay_cli --benchmark ...   # desktop pre-flight of the CPU series
```

### C.5 File-ownership table

No two packages edit the same file. The contract (`p2/contract`) is one commit that all four other branches descend from, so the integrator's rebase never sees an edit on both sides of a file.
...
| `MobileGL/MG_State/GLState/{VertexArrayState/VertexArrayState.*, FramebufferState/*, TextureState/*, BufferState/*, SamplerState/*}` | – | **owner** | – | – | – |
...
| `MobileGL/MG_Test/Pipe/CMakeLists.txt` + the four test stubs' creation | **owner** (contract) | – | – | – | – |
...
| `docs/Disaggregated/*.md` | – | – | – | – | – (integrator) |

...
> 状态：**P0、P0.5、P1、P2、P3a、P4a、P5、P5b、P5c、P5d 已收官**（P5c：2026-09-17，代码头 `b88e8487`；P5d：三轮，2026-09-18，代码头 `1f8de61b`）。当前头、逐门数字、开放项与下一步见 **[`CURRENT_STAGE_PROGRESS.md`](CURRENT_STAGE_PROGRESS.md)**；设计见 `ARCHITECTURE.md`；逐阶段实测见 `MEASUREMENTS.md`；P5d 报告见 [`P5D-INPROC-PERFORMANCE.md`](P5D-INPROC-PERFORMANCE.md)，P5e 报告见 [`P5E-RUNAHEAD.md`](P5E-RUNAHEAD.md)。**当前在 P5e**（退役 Espryt draw path 的 lockstep，契约 `MobileGL/MG_Remote/CONTRACT-P5E.md`；十二包已全部落地合并，**run-ahead 已武装**，头 `25fba0d5`；strict 车道硬绿 179/179、`integration-gpu` 1357/1357、三个构建 flavour 全绿；设备上 inproc 已与 monolith 齐平，见 `CURRENT_STAGE_PROGRESS.md` §2.6 与 `MEASUREMENTS.md` §11）；之后才是 P6 spawn transport（此时只是传输替换）。
...
| **P5e** 退役 Espryt draw path 的 lockstep | 🚧 进行中（十二包全落地，**run-ahead 已武装**，头 `25fba0d5`） | 契约 `MG_Remote/CONTRACT-P5E.md`（规则 F：**未设障**记录的 apply 不得读任何 client 内存；ID-84 限定它只约束未设障记录）；裁定 **ID-80..136**（`~/w7/notes/p5e/INTEGRATOR-DECISIONS-P5E.md`）。**十二个包，三波**：wave 1 **c0e**（契约 + 线上行：`PipeCalls.def` 第五列 `WaitClass`、`set_program_bindings` opcode 80、`kCapRunAheadApply` 位 10 仅 DirectGLES 臂、`kDrawClientArrays`、子系统位 13、两个旋钮）→ **id**（registry 按 `{slot, gen}` 重键、by-handle resolver、分配器守卫）；wave 2 **vi / sb / pg / tx2 / fb** 五个 per-draw 家族 ∥ **ra**（等待规则、present credit、`gPipeInputs` 变 server 角色内存），加 **fix1**（设备发现的 monolith 臂回归，ID-107）；wave 3 **pa**（program 家族离开前端）、**mv**（multi-draw + 五个档位的门禁条目）、**gl**（门禁机制本身 + 两处潜伏崩溃）、**ra2**（翻开关后暴露的竞态）。**`kMGPipeP5eRunAheadReady` 与 `kMGPipeP5eClientWaitRuleLanded` 均已翻**。最大的那次迁移（逐 draw 的 `GetProgramForDraw`）**没有改动任何线上结构**：记录早就带着逐 link 的 `ProgramArchive`，缺的是仍去问前端对象的读者。报告 [`P5E-RUNAHEAD.md`](P5E-RUNAHEAD.md) | **已达成**（头 `25fba0d5`，run-ahead 已武装）：`unit` 2256/2256；`integration-split` **179/179**；`integration-gpu` **1357/1357**（ID-109 新增的常设步，覆盖 push 构建的**第二条运行时臂**）；`flavours` rc=0（ID-124：pull / push / split 三个，且每臂**断言生效的编译宏**而不看 flag）；`integration-split-strict` **硬绿**——逐条普查 179/179、`Fatal{` 为零、`Admitted{` 恰为八行，白名单**生成**而非手抄（ID-116/125/128 的三个析取项）、车道是**两侧棘轮**；两条预期红车道各自具名断言。设备（Redmi，CPU 定频且回读验证，臂交错）：**VD32（~3550 draws/帧，两臂都远离上限）inproc 与 monolith 齐平**（p50 中位 59.9 vs 58.5，client CPU 15.72 vs 16.19 ms/帧，每臂五次），相对它取代的 lockstep **p50 +69%**。**尚未执行**：§3 item 4 的三条阴性对照、item 5 的逐包 red-once 重跑（其证据形式当时**不可执行**，ID-121 已重排到车道变绿之后）；`split_negative_controls.sh` 的 E1 对照自检就失败且所索 Fatal 在默认配置下不可能触发（ID-122）；G1 仍由 CI 断言（ID-123）；契约 §3.2（per-role stamp 存储）与 §8 修正 8 标为 UNLANDED。**已作废的判据**：原定的“`rsp` = 0”按构造就是空的（ID-119 改写）；“四臂设备出口”中的 `VERB_BARRIER=0` 不得做配对对照（ID-114，它会顺手关掉 run-ahead） | P5d |

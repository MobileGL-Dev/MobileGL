...
> 状态：**P0、P0.5、P1、P2、P3a、P4a、P5、P5b、P5c、P5d 已收官**（P5c：2026-09-17，代码头 `b88e8487`；P5d：2026-09-18，代码头 `56a77348`）。当前头、逐门数字、开放项与下一步见 **[`CURRENT_STAGE_PROGRESS.md`](CURRENT_STAGE_PROGRESS.md)**；设计见 `ARCHITECTURE.md`；逐阶段实测见 `MEASUREMENTS.md`；P5d 报告见 [`P5D-INPROC-PERFORMANCE.md`](P5D-INPROC-PERFORMANCE.md)。**下一个是 P6 spawn transport**（此时只是传输替换）。
...
两条跑道分开：**monolith 跑道** P0 → P0.5 → P1 → P2 → P3a → P4a → P3b/P4b → P7 → P8 → P13，每段可独立交付、可随时中止；**IPC 跑道** P5 → P5b → **P5c** → P6 → P9 → P10 → P11 → P12。目标顺序：先完整的 separate-thread rendering（P5b，已达成），再让 `inproc` 变成只经 wire 交换的诚实两角色（P5c），最后才是 separate-process transport（P6，此时只是传输替换）。
...
| **P5d** `inproc` 性能专项 | ✅ `cb06538c`、`56a77348` | persistent-map 推送三级臂（mprotect 脏页追踪：SIGSEGV 三重归属判定 + 内收页对齐 + 边块哈希 + 首推全量；哈希抑制 `MOBILEGL_IPC_PERSISTENT_HASH_SUPPRESS`；绑定过滤 `PushDrawConsumers` 含 TexBuffer——云闪修复）；barrier 批处理 `MOBILEGL_IPC_BATCH_WAITS` + 延期销毁队列 + 两条测试围栏；`Doorbell::Wait` 自旋时钟 64 合 1；零页注册 + shadow 页对齐分配（`SHADOW_ALLOCATION_ALIGNMENT=4096`，GL 查询仍 64）；epoch skip 稳态消掉绑定游走 | VD12 Minecraft（Redmi，FCL）split 7-13 → **63-65 fps**，monolith 110-118 同场景（~1:9 → ~1:1.7）；pmap 25 MB/帧 → 0.23 MB/帧；unit 2188/2188、`integration-split` 111/111；双生成器绿。未完成：后端 sync 锁复合（apply 线程 ~48% 在锁上）、R-1 序列化（P11）、emutls / set 去重哈希小项。报告：[`P5D-INPROC-PERFORMANCE.md`](P5D-INPROC-PERFORMANCE.md) | P5b |
| **P6** spawn transport | P5c 之后 | `SocketTransport`（socketpair + fork/execve，envp 剔除 + 强制 monolith 双保险）；`ServerMain`；`MOBILEGL_IPC_SERVER_PATH` + `dladdr` 兜底；有界重试握手；EOF 即时退出；device-lost latch；EGL forwarder 的控制面帧；P5c 留下的 `s_synced` / `g_syncedRenderStateParameters` 按 context 世代重置 | P5b 的完整渲染路径在 `spawn` 下绿；进程树只多一个子进程；`HeadlessGL` fork 预检无孤儿；OpenRA 在 Adreno 830 上 split SSIM ≥ 0.99 |

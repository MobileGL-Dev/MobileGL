...
| **P6** spawn transport | **P5f 前提已解除，尚未实施**；a6 / c6 待开始 | `SocketTransport`（socketpair + fork/execve，envp 剔除 + 强制 monolith 双保险）；`ServerMain`；`MOBILEGL_IPC_SERVER_PATH` + `dladdr` 兜底；有界重试握手；EOF 即时退出；device-lost latch。消费 P5f 已落地的值控制帧与 server-owned 状态，不重做 EGL 帧化或静态世代隔离。依赖序见 [`P6-SPAWN-PLAN.md`](P6-SPAWN-PLAN.md)：**a6**（核验进程装配/链接边界）→ **c6**（契约）→ **so / sm / cp / st / t6**。契约草稿 [`P6-CONTRACT-DRAFT.md`](P6-CONTRACT-DRAFT.md)。边界仍为 pbuffer / surfaceless / 离屏；真窗口到达归 P12。需区分不等待的 client 与已死亡的 server，不以超时阈值代替 device-lost 判定 | P5b 的完整渲染路径在 `spawn` 下绿；进程树只多一个子进程；`HeadlessGL` fork 预检无孤儿；OpenRA 在 Adreno 830 上 split SSIM ≥ 0.99；`integration-spawn` 与 `integration-split` 名集合一致；每条私有日志证明子进程 pid 与 `transport=spawn`；socket 门铃代价实际测量，性能只记录 |
...
| 未迁移与仿真路径 | 15 个 class-C 槽（query / sync 尾 / `GetTexImage` / `SetSwapInterval`）→ P9 / P10；client vertex arrays、multi-draw client indices、RGB 三通道 CPU mip、renderbuffer copy endpoint、未绑定 named clear、`texture-remint-pull` 仿真保留具名拒绝 → P8 / P9；Magma split compute / image 的“82 个错答”是 P5b 历史普查计数，不是当前失败清单；P5f 后续已补应用 buffer consumers 与 Android 旋转 blit，Magma inproc 已能运行所测 MC 世界；XFB、部分不对齐范围、placeholder/native-format 及性能仍归 P7（[报告](notes/p5f/magma-inproc-fix.md)） | **P5e 补**：client vertex arrays 在 run-ahead 下是**具名拒绝**（ID-82，`Fatal{UnmigratedVerb, "DrawArrays+CLIENT_ARRAYS"}`），并有自己的预期红车道 `integration-clientarrays-split`（ID-134）；staging 仍归 P8

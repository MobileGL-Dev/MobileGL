...
> 状态：**P0、P0.5、P1、P2、P3a、P4a、P5、P5b、P5c、P5d、P5e 已收官**（P5c：2026-09-17，代码头 `b88e8487`；P5d：三轮，2026-09-18，代码头 `1f8de61b`）。当前头、逐门数字、开放项与下一步见 **[`CURRENT_STAGE_PROGRESS.md`](CURRENT_STAGE_PROGRESS.md)**；设计见 `ARCHITECTURE.md`；逐阶段实测见 `MEASUREMENTS.md`；P5d 报告见 [`P5D-INPROC-PERFORMANCE.md`](P5D-INPROC-PERFORMANCE.md)，P5e 报告见 [`P5E-RUNAHEAD.md`](P5E-RUNAHEAD.md)。**P5e 已收官**（退役 Espryt draw path 的 lockstep，契约 `MobileGL/MG_Remote/CONTRACT-P5E.md`；十二包已全部落地合并，**run-ahead 已武装**，头 `25fba0d5`；strict 车道硬绿 179/179、`integration-gpu` 1357/1357、三个构建 flavour 全绿；设备上 inproc 已与 monolith 齐平，见 `CURRENT_STAGE_PROGRESS.md` §2.6 与 `MEASUREMENTS.md` §11）；出口门逐条已跑，唯一未过的是 E1 对照脚本本身，ID-122）。**当前在 P5f**（一切状态上 wire，计划 [`P5F-WIRE-COMPLETENESS.md`](P5F-WIRE-COMPLETENESS.md)）——它插在 P6 之前，因为“P6 只是传输替换”今天还不成立；P6 计划 [`P6-SPAWN-PLAN.md`](P6-SPAWN-PLAN.md)、契约草稿 [`P6-CONTRACT-DRAFT.md`](P6-CONTRACT-DRAFT.md)，均尚未开工）。
...
| **P5f** 一切状态上 wire | 📝 **计划已起草（2026-09-19）**，未开工 | 计划 [`P5F-WIRE-COMPLETENESS.md`](P5F-WIRE-COMPLETENESS.md)。**P6 的前提“只是传输替换”今天不成立**，本阶段把它变成成立：`FieldOwnership.def` 的 **21 行 / 15 个 `BARRIER_PULLED` 字段**归零（其中八个 P5e 只为 Espryt 退役、**Magma 一条未动**）；十二个 EGL forwarder 的控制面帧（P5c 审计 G4，原记 P6）；per-context 语义的进程级静态量；`MGPipeFrontendKeyedRegistryScope` 的约 12 个站点；反向通道中直接触碰 client 内存的部分。中心机制是**双块演练**：`inproc` 下给两个角色各一份 `PipeInputs`，把拆进程的语义在不拆进程的前提下先跑一遍 | 双块车道 `integration-split` 全绿；`strict-expected-markers.txt` **为空**且两侧棘轮仍在；`FieldOwnership.def` 零 `BARRIER_PULLED` 行、`rsp` 逐帧读零；Magma 与 Espryt 跑同一套判据；关掉双块必须有具名用例由绿转红 |
| **P6** spawn transport | ⏸ **阻塞于 P5f**；计划与契约草稿已起草（2026-09-19） | `SocketTransport`（socketpair + fork/execve，envp 剔除 + 强制 monolith 双保险）；`ServerMain`；`MOBILEGL_IPC_SERVER_PATH` + `dladdr` 兜底；有界重试握手；EOF 即时退出；device-lost latch；EGL forwarder 的控制面帧；P5c 留下的 `s_synced` / `g_syncedRenderStateParameters` 按 context 世代重置。包与依赖序见 [`P6-SPAWN-PLAN.md`](P6-SPAWN-PLAN.md)：**a6**（只读审计，先跑）→ **c6**（契约 + 惰性线上行）→ **so / sm / cp / st / t6** 并行。契约草稿 [`P6-CONTRACT-DRAFT.md`](P6-CONTRACT-DRAFT.md)（规则 G：**角色不得指名进程局部句柄**）。**落地边界**：P6 只落 pbuffer / surfaceless / 离屏（够跑 `HeadlessGL`、等价 split 车道与 trace replay）；`ANativeWindow*` 在子进程里无意义，**真窗口的到达是 P12**，到达 server 时具名拒绝。新设计只有一处：P5e 之后客户端不再等待，而对端进程会死——闩锁语义照抄 `ARCHITECTURE.md:504`，新的是“慢与死如何区分”（不得用超时阈值） | P5b 的完整渲染路径在 `spawn` 下绿；进程树只多一个子进程；`HeadlessGL` fork 预检无孤儿；OpenRA 在 Adreno 830 上 split SSIM ≥ 0.99；**G2**：`integration-spawn` 的用例名集合与 `integration-split` 逐名相同（名集合不同即意味着偷偷少跑）；**臂证明门**：每条用例在自己的私有日志里留下子进程 pid 与 `transport=spawn`（ID-124 已证明这类假绿真会发生，而一条实际跑了 monolith 的 spawn 车道能通过其余所有门）；性能只记录不阻塞，但 **socket 门铃相对 inproc condvar 的代价必须真采到**（P6 唯一预期的回归） |
...
| **P11** persistent map 与 ≥16 MiB 采纳 | 待排 | POST 探针档位选择（T0 主攻，Adreno 可选 T1，T2 回退）；`SEG_ADOPT` 生命周期绑 `completedFrameSerial`；**`gPipeInputs` 版本化 / 双缓冲**（P5d 研究：draw barrier 延后一个 verb 的前提，须在 P3b/P4b 的句柄键控 twin 表之后）。**P5e 的更正**：P5e 退役 draw lockstep **没有**需要它——ra2 实测证明字段**值**已由 `SuppliedFieldMask` 分区，当时那次竞态在**元数据标量**上，修法是让守卫测事实而非断言未来（ID-135）。本行对一般情形仍然成立，且契约 §3.2 的 per-role stamp 存储仍未落地 | `LargeArenaAdoptionScenario` 在所选档下绿；26.3 与两个 Create fixture SSIM ≥ 0.99；Adreno 830 上 p99 帧时与峰值 RSS 对采纳基线（163→21 ms / 40→115 fps / ~400 MB）回归不超过 10% |
...
对象类 BARRIER-PULLED 行与 twin registry 的前端键（P3b/P4b、P7）；XFB scatter / `OnXfbScatterReady`、`OnTexturePullRequest` 与终止符、`OnGlError` 有序化（P9）；EGL forwarder 的控制面（原记 P6，**P5f 已收回为包 fc 并落地**，见 G4 行）；`copy-image-shadow-mirror` 与 CopyTex 纹素回写（P8 / P9）；`s_synced` / `g_syncedRenderStateParameters` 的 context 世代重置（P6）。
...
| `rsp` residual inputs | **P5e 已重写这条的口径**：`integration-split-strict` 现为硬绿车道（逐条普查 179/179、`Fatal{` 为零、`Admitted{` 八行，均有退役相）。原先的“未设障记录 `rsp` = 0”**按构造就是空的**：`CountBarrierPull` 的未设障臂在 `++g_residualPulls` 之前且 `[[noreturn]]`，所以 `rsp` 只数**设障**的 pull；ID-119 改写为“`rsp` 只数被采纳的设障 pull，且等于被采纳标记的计数”。剩余八行分属 P7 / P8 / P9 / P3b•P4b |

...
分支 `feat/disaggregated`，头 `7cb29d46`（2026-09-16，已推 GitHub）。本文随每次落地更新。
...
| **P5b** inproc 下的 verb 迁移 | **代码已落地，测量未收口** | `37fc4fdb..7cb29d46` |
| P6 spawn transport | 未开始 | 排在 P5b 之后 |
...
| unit | pull / push / verify 各 1817/1817；split 2138/2138 |
| `integration-split`（inproc） | **107/107**（P5 收官时 22） |
| `integration-gpu`（push） | 1148/1148 |
| `integration-gpu`（split，单体传输） | 1267/1267 |
| G5（p3a / p4a 保护区） | 双绿 |
| 集成普查（inproc，测于 `348d22a4`） | 811 passed / 203 skipped / 62 aborted，共 1267 |
...
| 项 | 证据 |
|---|---|
| `SEG_STAGE` 默认 32 MiB 装不下 128 MiB 上传：improved-transparency DirectGLES 报 `Fatal{RingOverrun}`。`MOBILEGL_IPC_STAGE_MB=256` 是 profile，不是修复 | `p5b-results/blit-codex-v1.md` |
| Magma（DirectVulkan）split compute/image 路径：89 个错答中 82 个 | `p5b-results/i1-v1.md` |
| rd12 DirectGLES `InitialBytesNotCarried/resource_respecify`；rd12 DirectVulkan `BarrierTimeout/Present` | `p5b-results/d1-codex-v1.md` |
| RGB 三通道 CPU mip 回退 | `p5b-results/mip-codex-v1.md` |
| 设备 barrier tax 未测（此前每次 split 臂都停在 `DrawElements`） | `p5-results/ab-v1.md` |
...
1. 新头上跑一次完整普查（车道 + 79 个 trace），拿到真实的 Minecraft 渲染数与新的首阻塞表。
2. 修 `SEG_STAGE` 默认容量。
3. 重建三个 APK（pull / push / split），跑红米四臂 A/B：首次可测 barrier tax。
4. host 全门一次 + 文档收口 → P5b 收官。
5. 剩余首阻塞（Magma compute/image、rd12、RGB mip）一轮。
6. P6 spawn transport。
...
## 8. 裁定索引（ID-40..75；ID-1..39 见 git 历史）

...
- **读点覆盖**（`python3 scripts/gen_pipe.py`）：71 条调用（11 screen / 60 context）、63 个 verify payload、**63** 个 `PipeInputs` 字段（旧数 61 是 P1 之前的，`gen_pipe.py` 自己打印 63）；477 行后端读点清单 → 299 调用、5 client 自答、6 反向通道、167 结构性句柄、**0 UNMAPPED**。

...
**7.1 门 3 分母（ID-P7-4）。** `trace_cases.json` 的 CI split 子集 × DirectVulkan × `--use-pbuffer`，排除 `minecraft-1.21.4-rd12-odinlite-in-world` 与 `create-indirect`（OQ-17：`dev@81b17c0b` 起在 Adreno 830 上就坏）；`iterationrp` 带 `apk.yml:460-462` 的三个 Magma 旋钮。
**7.2 门 3 判据。** 同一 reboot-clean 会话内：monolith 对照 1.0；inproc **与** spawn（或 tcp）三遍 ≥ 各 case 现有阈值、OpenRA 1.0、三遍逐位相同（零翻转）；`RUN_AHEAD=0` 对照臂一遍记录。证据目录 `docs/Disaggregated/notes/p7/device-window-<n>/`。定位实验顺序 E0–E6 见计划 §3 wave 1。
**7.3 门 5。** 块：`KHR-GL46.texture_*`、`shader_image_*`、`packed_pixels`、`direct_state_access.*`、`uniform_buffer_object*`；caselist 入库 `tools/cts/caselists/p7-*.txt`（记 suite 版本）；先 monolith × DirectVulkan 基线入库 `tools/cts/baselines/`，迁移落地后 inproc × DirectVulkan 比对：每块 pass 率差 ≤ 0.5 pp（Pass/(Pass+Fail)，NS 不入分母），**新增 crash = 0 单独硬红**。完整 caselist（~56k）在合 `dev` 前跑，不在关键路径。

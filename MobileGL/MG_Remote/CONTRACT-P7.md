...
| `copy-image-in-place` | `VulkanRenderer.cpp` | **已退役（wave 2-B，`9e126a14`）** | `srcImage == dstImage` 时单次转 `GENERAL`、两侧布局 GENERAL、一次写回；同 mip+layer 且矩形重叠 → decline（GL 未定义） |
| `mipmap-shader-format-or-shape`（真机 iterationt 的死因） | `WireFramebuffer.inc` | **半退役（wave 2-B，`c5fa52f9`）** | Fatal 行带上格式 / 尺寸 / aspect / 缺的 feature 位；image 不能作附件时经 scratch 走颜色 shader mip；仍无 shader 臂的形状**具名**拒绝 |
| **wire 回读竞态（OpenRA 真机 0.976494 的原因）** | `WireFramebuffer.inc` / `VulkanRenderer.cpp` | **已修（wave 2-B，`64530ed2`）** | `ReadWirePixels` 在 pooled fence 下 flush 帧内 draw 但不等待，拷贝是后一次提交，二者只靠一条 barrier 相连，而 `UNDEFINED` / `PRESENT_SRC_KHR` 的源 scope 映射成 `TOP_OF_PIPE`+0 = 空 scope——刚 present 过的默认 framebuffer正是这个布局；修后回读等它读的那一帧（red-once：`fences=2` vs `1`，`completed=71` 不变）。真机验收 = 三次 `pm clear` 冷跑 1.0 |
...
| `default-color-blit-shape` | `WireFramebuffer.inc` | **已退役（wave 2-B，`0a8a7a0c`）** | 恒等变换回落 `vkCmdBlitImage` 臂；非恒等旋转先 resolve/copy 进 scratch 2D RGBA 再 shader blit；四旋转之外 decline。顺手修出拒绝背后的活 bug：普通 blit 臂对 3D 源按 layer 寻址（z 0..1 读到 slice 0）。旋转半边只在真机可证 |
| `multisample-blit-shape` / `-aspect` | `WireFramebuffer.inc` | **未做（B 剩余 2 处 `@P7`）** | shape：resolve 进 scratch 单采样再 `vkCmdBlitImage` 缩放/翻转；aspect：MS 深/模板 → 单采样用 `VK_KHR_depth_stencil_resolve`（`DynamicBackendParameters` 里查扩展）或 `texelFetch(sampler2DMS)` 写 `gl_FragDepth` 的 shader resolve；单采样 → MS 尺寸不同 decline |
...
- **`TextureViewAliasScenario.TheOwnerUploadDrainRunsAgainAfterADrawBoundary` 在三条 split 臂按名排除**（D2 的新门发现的产品缺陷：view 被采样过之后，owner 上的 `glTexSubImage` 再也不经 view 可见——`Managers.cpp:15433` 读 view 自己的 `record.Serial`，owner 侧 `resource_subdata` 只推 owner 的（`PipeApply.cpp:1175`），`Desc.ViewOf` 无反向索引；`CONTRACT-P5E.md:387-395` 写的就是这个洞）。主人 **D3**；落地时删排除、改 P5E §5.2 的 clean-gate 句。排除同时把它从 tier-2 Magma 回放里拿掉了（`MGL_SPLIT_ARM_CALLS` 共用过滤）。
- **bsl-esc-menu-854 在 wire 臂上是内存缺陷不是排除项**（W4 §2/§3c）：monolith 807 MiB 峰值通过；spawn 下 server 进程涨到 ~1.2 GiB 在 scudo 二级分配器 `internal map failure (Out of memory)` 死（系统尚余 8 GB → 是映射数或每进程上限，不是系统 OOM），client 同时 543 MiB；inproc 1.6 GiB 也死。归属交 **M1** 调查（store 逐项归因、泄漏还是 2× 结构重复、`vm.max_map_count`）；与 B3 的「`UploadPendingWireLevels` 拒绝后 pending 条目存活」可能同根。

...
- P3b/P4b 的 CTS AFTER → 该阶段。Iris 普查的 P9 例外（`texture-remint-pull`）：wave 3 包 E1 在 `1135c664` 的主机普查里**为空**——77 行 × {monolith, inproc, spawn} 231 次全活，该站点一次未到达（`docs/Disaggregated/notes/p7/iris-census-1135c664.md` §4）；设备侧读数留 wave 4。
...
- ~~`MEASUREMENTS.md` §7.2 的同名比对自 P5b 起未重跑；「22 → texture readback」「3 → query」两行债务的关闭以 wave 3 的重跑为准。~~ **已退（wave 3 包 E1，`1135c664`）**：同名重跑 27 条 inproc 1 passed / 26 failed → 24 passed / 3 aborted / 0 failed（spawn 同）。22 条 readback 中 19 绿；3 条是 `MOBILEGL_PIPE_PUSH=0` 对照臂的具名停止（`PipeLegacyMemosDisabled` / `Magma:clear-framebuffer-record`），不是错答；3 条 query 全绿（`MEASUREMENTS.md` §7.2 末）。
...
- **F 片 1–3 之后（`notes/p7/ph-f.md` §4，ID-P7-37）**：`wireFingerprint` 随 `Welcome.dataNonce` / `DataBind` 变更（`0x7d4470f241d23539` → `0x03c4c293a54b3e08`），旧 server 与新 client 按指纹互拒——设备上的 server 必须随下一个 APK 重部署；D11 五处 + PH-2、PH-6、PH-1 (3)(4)、PH-7 (5)（fork 前认证）未做，前置是一个能向 spawn / TCP server 投递畸形记录的对端字节驱动（fuzz 臂 2 的第一块），包 **F2**。
